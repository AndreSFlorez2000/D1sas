...
function normalizeSecondaryRow(row, index) {
  ...
  return {
    ...
    presupuesto:Number(String(find('Presupuesto','Valor','Costo')||'').replace(/[^0-9.-]/g,''))||0,
    ...
    descripcionTecnica:String(find('Descripción técnica','Descripcion tecnica','Descripción Técnica','Descripcion Técnica')||'').trim(),
    fechaEntrega:find('Fecha Entrega','Fecha de entrega')||'',
    fechaTerminoTienda:find('Fecha Término tienda','Fecha Termino tienda')||'',
    fechaTermino:find('Fecha de Término','Fecha de Termino','Fecha término','Fecha termino')||'',
    fechaIdealTermino:find('Fecha Ideal de Término','Fecha Ideal de Termino')||'',
    fechaAceptada:find('Fecha Incidencia Aceptada','Fecha aceptada')||'',
    estadoCotizacion:String(find('Estado Cotización','Estado Cotizacion')||'').trim(),
    estadoEjecucion:String(find('Estado Ejecución','Estado Ejecucion')||'').trim(),
    jefeConforme:String(find('¿Jefe de Tienda Conforme?','Jefe de Tienda Conforme')||'').trim(),
    cerradoPorSupervisor:String(find('Cerrado por supervisor')||'').trim(),
    anulacion:String(find('Anulación','Anulacion')||'').trim(),
    rechazo:String(find('Rechazo')||'').trim(),
    cambioProveedor:String(find('Cambio proveedor')||'').trim(),
...
function isD1goClosed(row) {
  const e = normalizeText(row?.estadoGeneral || '');
  return e.includes('cerrad');
}

function isD1goActive(row) {
  const e = normalizeText(row?.estadoGeneral || '');
  return e.includes('abiert') || e.includes('solicit') || e.includes('pendient');
}

function daysBetweenDates(start, end) {
  if (!start || !end || !(start instanceof Date) || !(end instanceof Date) || isNaN(start) || isNaN(end)) return null;
  return Math.max(0, Math.round((end - start) / 86400000));
}

function getD1goReferenceDate(rows) {
  const dates = (rows || []).flatMap(r => [r.fechaIngresoDate, r.fechaCierreDate]).filter(d => d instanceof Date && !isNaN(d));
  if (!dates.length) return new Date();
  return new Date(Math.max(...dates.map(d => d.getTime())));
}

function getD1goAgeDays(row, referenceDate) {
  if (isD1goClosed(row)) return daysBetweenDates(row.fechaIngresoDate, row.fechaCierreDate);
  return daysBetweenDates(row.fechaIngresoDate, referenceDate || new Date());
}

function getD1goAlertLevel(row, referenceDate) {
  const days = getD1goAgeDays(row, referenceDate);
  if (days === null) return { level: 'sin_fecha', label: 'Sin fecha', days: '' };
  if (days > 60) return { level: 'roja', label: '+60 días', days };
  if (days > 30) return { level: 'naranja', label: '+30 días', days };
  return { level: 'ok', label: 'Normal', days };
}

function getPrimaryOcMapForD1go() {
  const map = new Map();
  (state.rowsByView.sheet1 || []).forEach(row => {
    if (!row.incidencia || row.isDeleted) return;
    const cur = map.get(row.incidencia);
    const rank = getPrimaryStateRank(row.viewGroup);
    if (!cur || rank > cur.rank) {
      map.set(row.incidencia, {
        rank,
        estadoOc: getPrimaryStateLabel(row.viewGroup),
        viewGroup: row.viewGroup,
        oc: row.oc || '',
        proveedor: row.proveedor || '',
        tiendaPrincipal: row.tienda || '',
        valor: Number(row.rawPrecioNeto || 0),
        porEntregar: Number(row.rawPorEntregarValor || 0),
        xLiberacion: Number(row.xLiberacion || 0)
      });
    }
  });
...
function buildD1goFullAnalysis(rows) {
  const ref = getD1goReferenceDate(rows);
  const ocMap = getPrimaryOcMapForD1go();
  const analyzed = (rows || []).map(r => {
    const oc = ocMap.get(r.id) || null;
    const alert = getD1goAlertLevel(r, ref);
    return { ...r, ocMatch: oc, alertLevel: alert.level, alertLabel: alert.label, ageDays: alert.days };
  });
  const total = analyzed.length;
  const cerradas = analyzed.filter(isD1goClosed).length;
  const activas = analyzed.filter(isD1goActive).length;
  const sinFecha = analyzed.filter(r => r.alertLevel === 'sin_fecha').length;
  const alert30Activas = analyzed.filter(r => isD1goActive(r) && r.alertLevel === 'naranja').length;
  const alert60Activas = analyzed.filter(r => isD1goActive(r) && r.alertLevel === 'roja').length;
  const cerradas30 = analyzed.filter(r => isD1goClosed(r) && r.alertLevel === 'naranja').length;
  const cerradas60 = analyzed.filter(r => isD1goClosed(r) && r.alertLevel === 'roja').length;
  const cruzadas = analyzed.filter(r => r.ocMatch).length;
  const cruzadasFactAbiertas = analyzed.filter(r => r.ocMatch && r.ocMatch.viewGroup === 'facturadas' && isD1goActive(r)).length;
  const cruzadasPendAbiertas = analyzed.filter(r => r.ocMatch && ['pendientes','activos','sheet1'].includes(r.ocMatch.viewGroup) && isD1goActive(r)).length;
  const validClosedDays = analyzed.filter(r => isD1goClosed(r) && typeof r.ageDays === 'number').map(r => r.ageDays);
  const promCierre = validClosedDays.length ? validClosedDays.reduce((a,b)=>a+b,0)/validClosedDays.length : 0;
  return { ref, rows: analyzed, total, cerradas, activas, sinFecha, alert30Activas, alert60Activas, cerradas30, cerradas60, cruzadas, cruzadasFactAbiertas, cruzadasPendAbiertas, promCierre };
}

function buildD1goKpiCards(analysis) {
  const pctCierre = analysis.total ? (analysis.cerradas / analysis.total) * 100 : 0;
  const pctCruce = analysis.total ? (analysis.cruzadas / analysis.total) * 100 : 0;
  return `
    <div class="d1go-insight-grid">
      <div class="d1go-insight-card"><small>% cierre</small><strong>${pctCierre.toFixed(1)}%</strong><span>${formatNumber(analysis.cerradas)} de ${formatNumber(analysis.total)} cerradas</span></div>
      <div class="d1go-insight-card danger"><small>Activas +60 días</small><strong>${formatNumber(analysis.alert60Activas)}</strong><span>Abiertas / solicitadas / pendientes</span></div>
      <div class="d1go-insight-card warning"><small>Activas +30 días</small><strong>${formatNumber(analysis.alert30Activas)}</strong><span>Requieren seguimiento</span></div>
      <div class="d1go-insight-card"><small>Promedio de cierre</small><strong>${analysis.promCierre.toFixed(1)} días</strong><span>Solo incidencias cerradas con fecha</span></div>
      <div class="d1go-insight-card"><small>Cruzadas con OC</small><strong>${formatNumber(analysis.cruzadas)}</strong><span>${pctCruce.toFixed(1)}% del D1GO visible</span></div>
      <div class="d1go-insight-card danger"><small>Facturadas aún activas</small><strong>${formatNumber(analysis.cruzadasFactAbiertas)}</strong><span>Cruce crítico OC facturada + D1GO activo</span></div>
      <div class="d1go-insight-card warning"><small>OC pendiente/activa + D1GO activo</small><strong>${formatNumber(analysis.cruzadasPendAbiertas)}</strong><span>Casos cruzados para seguimiento</span></div>
      <div class="d1go-insight-card"><small>Cerradas demoradas</small><strong>${formatNumber(analysis.cerradas30 + analysis.cerradas60)}</strong><span>${formatNumber(analysis.cerradas60)} cerraron después de 60 días</span></div>
    </div>`;
}

function buildD1goAlertTable(title, rows, limit=20) {
  const data = (rows || []).slice().sort((a,b)=>(Number(b.ageDays)||0)-(Number(a.ageDays)||0)).slice(0,limit);
  return `<div class="stats-card-block"><h4>${escapeHtml(title)}</h4><div style="overflow:auto;"><table class="stats-table" style="min-width:980px;"><thead><tr><th>ID</th><th>Estado</th><th>Alerta</th><th>Días</th><th>Ingreso</th><th>Cierre</th><th>Tienda</th><th>Supervisor</th><th>Servicio</th><th>OC</th></tr></thead><tbody>${data.length?data.map(r=>`<tr><td>${escapeHtml(r.id||'—')}</td><td>${escapeHtml(r.estadoGeneral||'—')}</td><td><span class="d1go-alert-pill ${escapeHtml(r.alertLevel)}">${escapeHtml(r.alertLabel||'—')}</span></td><td>${escapeHtml(String(r.ageDays||'—'))}</td><td>${escapeHtml(formatDateValue(r.fechaIngreso)||'—')}</td><td>${escapeHtml(formatDateValue(r.fechaCierre)||'—')}</td><td>${escapeHtml(r.sucursal||'—')}</td><td>${escapeHtml(r.supervisor||'—')}</td><td>${escapeHtml(r.servicio||'—')}</td><td>${escapeHtml(r.ocMatch?.oc||'—')}</td></tr>`).join(''):'<tr><td colspan="10">Sin datos</td></tr>'}</tbody></table></div></div>`;
}

function exportD1goFilteredData() {
  const rows = getVisibleD1goRows();
  if (!rows.length) { showInfoDialog('Sin datos','No hay registros D1GO visibles para exportar.'); return; }
  const analysis = buildD1goFullAnalysis(rows);
  const data = analysis.rows.map(r => ({
    ID: r.id,
    Estado_General: r.estadoGeneral,
    Estado_Cotizacion: r.estadoCotizacion,
    Estado_Ejecucion: r.estadoEjecucion,
    Alerta: r.alertLabel,
    Dias: r.ageDays,
    Fecha_Ingreso: formatDateValue(r.fechaIngreso),
    Fecha_Cierre: formatDateValue(r.fechaCierre),
    Fecha_Entrega: formatDateValue(r.fechaEntrega),
    Fecha_Termino_Tienda: formatDateValue(r.fechaTerminoTienda),
    Fecha_Termino: formatDateValue(r.fechaTermino),
    Fecha_Ideal_Termino: formatDateValue(r.fechaIdealTermino),
    Sucursal: r.sucursal,
    Area: r.area,
    Servicio: r.servicio,
    Tipo: r.tipo,
    ...
    Supervisor: r.supervisor,
    Proveedor_Adjudicado: r.proveedorAdjudicado,
    Contratista: r.contratista,
    Presupuesto: r.presupuesto,
    Especialidad: r.especialidad,
    OC_Cruzada: r.ocMatch?.oc || '',
    Estado_OC: r.ocMatch?.estadoOc || '',
    Valor_OC: r.ocMatch?.valor || '',
    Por_Entregar_OC: r.ocMatch?.porEntregar || '',
    Detalle: r.detalle,
    Descripcion_Tecnica: r.descripcionTecnica,
    Anulacion: r.anulacion,
    Rechazo: r.rechazo,
    Cambio_Proveedor: r.cambioProveedor
  }));
  const wb2 = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb2, XLSX.utils.json_to_sheet(data), 'D1GO_Completo');
  const resumen = [
    { Indicador: 'Total visible', Valor: analysis.total },
    { Indicador: 'Cerradas', Valor: analysis.cerradas },
    { Indicador: 'Activas', Valor: analysis.activas },
    { Indicador: 'Activas +30 días', Valor: analysis.alert30Activas },
    { Indicador: 'Activas +60 días', Valor: analysis.alert60Activas },
    { Indicador: 'Cerradas entre +30 y +60 días', Valor: analysis.cerradas30 },
    { Indicador: 'Cerradas +60 días', Valor: analysis.cerradas60 },
    { Indicador: 'Cruzadas con OC', Valor: analysis.cruzadas },
    { Indicador: 'Facturadas aún activas', Valor: analysis.cruzadasFactAbiertas },
    { Indicador: 'OC pendiente/activa + D1GO activo', Valor: analysis.cruzadasPendAbiertas },
    { Indicador: 'Promedio días cierre', Valor: Number(analysis.promCierre.toFixed(1)) }
  ];
  XLSX.utils.book_append_sheet(wb2, XLSX.utils.json_to_sheet(resumen), 'Resumen');
  XLSX.writeFile(wb2, 'D1GO_analisis_completo.xlsx');
  toast('Análisis D1GO completo exportado ✅');
}
...
function renderD1goStatsModal() {
  ...
  const porEstado=Array.from(buildCategoryCountMap(analyzedRows,r=>r.estadoGeneral).entries()).sort((a,b)=>b[1]-a[1]);
  const porSup=Array.from(buildCategoryCountMap(analyzedRows,r=>r.supervisor).entries()).sort((a,b)=>b[1]-a[1]);
  const porTienda=Array.from(buildCategoryCountMap(analyzedRows,r=>r.sucursal).entries()).sort((a,b)=>b[1]-a[1]);
  const porArea=Array.from(buildCategoryCountMap(analyzedRows,r=>r.area).entries()).sort((a,b)=>b[1]-a[1]);
  const porServicio=Array.from(buildCategoryCountMap(analyzedRows,r=>r.servicio).entries()).sort((a,b)=>b[1]-a[1]);
  const porPrioridad=Array.from(buildCategoryCountMap(analyzedRows,r=>r.prioridad).entries()).sort((a,b)=>b[1]-a[1]);
  const porProveedor=Array.from(buildCategoryCountMap(analyzedRows,r=>r.proveedorAdjudicado || r.contratista).entries()).sort((a,b)=>b[1]-a[1]);
  const porAlerta=Array.from(buildCategoryCountMap(analyzedRows,r=>r.alertLabel).entries()).sort((a,b)=>b[1]-a[1]);
  const porFecha=Array.from(buildCategoryCountMap(analyzedRows,r=>formatDateLabelFromKey(r.fechaIngresoKey)||'Sin fecha').entries()).sort((a,b)=>b[1]-a[1]);
  const porMesIngreso=Array.from(buildCategoryCountMap(analyzedRows,r=>formatMonthLabel(getMonthKeyFromDateKey(r.fechaIngresoKey)||'')||'Sin mes').entries()).sort((a,b)=>b[1]-a[1]);
  const porMesCierre=Array.from(buildCategoryCountMap(analyzedRows.filter(isD1goClosed),r=>formatMonthLabel(getMonthKeyFromDateKey(r.fechaCierreKey)||'')||'Sin mes').entries()).sort((a,b)=>b[1]-a[1]);
  const selectedDateKey=String(d1goFechaFilter.value||'').trim();
  const selectedMonthKey=String(d1goMesFilter?.value||'').trim();

  d1goStatsTotal.textContent=formatNumber(analysis.total);
  ...
  const activasCriticas = analyzedRows.filter(r => isD1goActive(r) && ['roja','naranja'].includes(r.alertLevel));
  const cerradasLentas = analyzedRows.filter(r => isD1goClosed(r) && ['roja','naranja'].includes(r.alertLevel));
  const cruzadasCriticas = analyzedRows.filter(r => r.ocMatch && isD1goActive(r));

  d1goStatsContent.innerHTML=[
    buildD1goKpiCards(analysis),
    buildSimpleStatsTable('Estados D1GO completos',porEstado.slice(0,15)),
    buildSimpleStatsTable('Alertas por antigüedad / cierre',porAlerta.slice(0,10)),
    buildSimpleStatsTable('Top áreas',porArea.slice(0,15)),
    buildSimpleStatsTable('Top servicios',porServicio.slice(0,15)),
    buildSimpleStatsTable('Top prioridades',porPrioridad.slice(0,15)),
    buildSimpleStatsTable('Top proveedores / contratistas',porProveedor.slice(0,15)),
    buildSimpleStatsTable('Supervisores D1GO',porSup.slice(0,15)),
    buildSimpleStatsTable('Tiendas D1GO',porTienda.slice(0,15)),
    buildSimpleStatsTable('Ingresadas por mes',porMesIngreso.slice(0,12)),
    buildSimpleStatsTable('Cerradas por mes',porMesCierre.slice(0,12)),
    buildD1goAlertTable('Activas más críticas (+30 / +60 días)', activasCriticas, 25),
    buildD1goAlertTable('Cerradas con mayor duración', cerradasLentas, 25),
    buildD1goAlertTable('Cruce OC + D1GO activo', cruzadasCriticas, 25),
    buildSimpleStatsTable(
      selectedDateKey
        ...
        : selectedMonthKey
          ...
          : 'Actividad por fecha',
      selectedDateKey
        ? [[`Incidencias del día`,analyzedRows.filter(r=>r.fechaIngresoKey===selectedDateKey||r.fechaCierreKey===selectedDateKey).length],[`Cerradas del día`,analyzedRows.filter(r=>r.fechaCierreKey===selectedDateKey&&isD1goClosed(r)).length]]
        : selectedMonthKey
          ? [[`Incidencias del mes`,analyzedRows.filter(r=>getMonthKeyFromDateKey(r.fechaIngresoKey)===selectedMonthKey||getMonthKeyFromDateKey(r.fechaCierreKey)===selectedMonthKey).length],[`Cerradas del mes`,analyzedRows.filter(r=>getMonthKeyFromDateKey(r.fechaCierreKey)===selectedMonthKey&&isD1goClosed(r)).length]]
          : porFecha.slice(0,15)
    )
  ].join('');
